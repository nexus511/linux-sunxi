...
struct shfl_query_mappings {
	VBGLIOCHGCMCALL call_info;

	/**
	 * 32bit, in:
	 * Flags describing various client needs.
	 */
	struct hgcm_function_parameter flags;
	 ...
	 * mappings array.
	 */
	struct hgcm_function_parameter number_of_mappings;

	/**
	 * pointer, in/out:
	 * Points to array of struct shfl_mapping structures.
	 */
	struct hgcm_function_parameter mappings;
...
struct shfl_query_map_name {
	 ...
	 */
	struct hgcm_function_parameter root;

	/**
	 * pointer, in/out:
	 * Points to struct shfl_string buffer.
	 */
	struct hgcm_function_parameter name;
...
struct shfl_map_folder {
	 ...
	 * Points to struct shfl_string buffer.
	 */
	struct hgcm_function_parameter path;
	 ...
	 * Path delimiter
	 */
	struct hgcm_function_parameter delimiter;

	/**
	 * pointer, in: SHFLROOT
	 * Case senstive flag
	 */
	struct hgcm_function_parameter case_sensitive;
...
struct shfl_create {
	 ...
	 */
	struct hgcm_function_parameter parms;
...
struct shfl_close {
	 ...
	 */
	struct hgcm_function_parameter handle;
...
struct shfl_read {
	 ...
	 * Offset to read from.
	 */
	struct hgcm_function_parameter offset;

	/**
	 * value64, in/out:
	 * Bytes to read/How many were read.
	 */
	struct hgcm_function_parameter cb;

	/**
	 * pointer, out:
	 * Buffer to place data to.
	 */
	struct hgcm_function_parameter buffer;
...
struct shfl_lock {
	 ...
	 * Length of range.
	 */
	struct hgcm_function_parameter length;
...
struct shfl_list {
	 ...
	 */
	struct hgcm_function_parameter resume_point;

	/**
	 * pointer, out:
	 * Number of files returned
	 */
	struct hgcm_function_parameter file_count;
};
...
struct shfl_information {
	 ...
	 * operation as well.
	 */
	struct hgcm_function_parameter info;
...
struct shfl_rename {
	 ...
	 */
	struct hgcm_function_parameter src;

	/**
	 * pointer, in:
	 * Points to struct shfl_string dest.
	 */
	struct hgcm_function_parameter dest;
...
struct shfl_symlink {
	 ...
	 */
	struct hgcm_function_parameter new_path;

	/**
	 * pointer, in:
	 * Points to struct shfl_string of destination for symlink.
	 */
	struct hgcm_function_parameter old_path;
